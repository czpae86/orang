...
(function(){
    ...
    var orang = {},
        root = this,
        fileExtName = [".js",".css",".html",".png",".gif",".ioc",".jpg",".jpeg"],
        handlerMapping = {},
    ...
    function filter(req,res){
        ...
        if(fileExtName.indexOf(extname)!=-1){
            ...
            if(len<=0){
                ...
                for(var i=0;i<len;i++){
                    var realPath = path.join(config.staticResource[i],pathname);
                    try{
                        var stats = fs.statSync(realPath);
                        tools.zipResponse(req,res,stats,realPath);
                        break;
                    }catch(e){
                        if((i+1)==len&& e.code=="ENOENT"){
                            return tools.return404(res);
                        }else if((i+1)==len){
